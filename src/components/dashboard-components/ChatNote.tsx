  ...
  return (
    <Drawer>
      <DrawerTrigger asChild>
        <Button variant="outline" className=" fixed bottom-5 right-5 lg:bottom-10 lg:right-10 border-Purple300 rounded-full bg-brand_secondary"><FaRegComments className=" lg:size-8 size-6 text-Purple700"/></Button>
      </DrawerTrigger>
      <DrawerContent>
        <div className="mx-auto w-full max-w-sm mb-10 mt-5">
          <DrawerHeader className=" *:w-full *:text-center space-y-2">
            ...
            <DrawerDescription className=" text-sm font-bold text-brand_primary/50"> {`We're currently fixing the chatbot; there's a problem with the OpenAI API.`}</DrawerDescription>
            ...
            <div className="flex-grow min-h-[250px] overflow-y-auto" >
              {renderResponse()}
                ...
                <form onSubmit={handleSubmit} className=" relative space-y-8">
                  ...
                  <button type="submit" className=" absolute -top-6 right-3 p-1 rounded-md bg-brand_primary cursor-pointer hover:opacity-50"><FaArrowTurnUp className=" text-brand_primary"/></button>
                </form>
            </div>
        </div>
      </DrawerContent>
