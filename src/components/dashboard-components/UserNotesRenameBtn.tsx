  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <p className=" text-brand_primary text-base font-semibold p-1 rounded-md transition-all cursor-pointer text-nowrap overflow-hidden hover:bg-brand_primary/5">{NoteName}</p>
      </DropdownMenuTrigger>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 px-2">
                ...
                render={({ field }) => (
                    <FormItem className="relative">
                        <FormLabel className=" text-sm font-bold text-brand_primary"> Rename :</FormLabel>
                    <FormControl>
                        <Input disabled={loading} className=" w-full" placeholder={`${NoteName} ...`} type="text" {...field} />
                    </FormControl>
                    {(loading)&&<span className=" absolute top-8 right-2 loading loading-infinity loading-md text-brand_primary/20"></span>} 
